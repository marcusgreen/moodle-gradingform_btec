...
M.gradingform_bteceditor.disablealleditors = function() {
    var Y = M.gradingform_bteceditor.Y
    var name = M.gradingform_bteceditor.name
    Y.all('#btec-'+name+' .criteria .description input[type=text]:not(.pseudotablink)').each( function(node) {M.gradingform_bteceditor.editmode(node, false)} );
    Y.all('#btec-'+name+' .criteria .description textarea').each( function(node) {M.gradingform_bteceditor.editmode(node, false)} );
    Y.all('#btec-'+name+' .comments .description textarea').each( function(node) {M.gradingform_bteceditor.editmode(node, false)} );
}
